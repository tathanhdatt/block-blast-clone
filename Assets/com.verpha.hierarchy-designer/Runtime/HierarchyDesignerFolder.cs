namespace Verpha.HierarchyDesigner
{
    ...
    public class HierarchyDesignerFolder : MonoBehaviour
    {
        ...
        [Tooltip(
            "Flatten Folder = Free all the folder's GameObject children in the Awake/Start method (FlattenEvent), then once the operation is complete, destroy the folder.")]
        [SerializeField]
        private bool flattenFolder = true;

        public bool ShouldFlatten => flattenFolder;

        public enum FlattenEvent
        {
            Awake,
            Start
        }
        ...
        private void RecursivelyFlatten(Transform folderTransform)
        {
            ...
            for (int i = folderTransform.childCount - 1; i >= 0; i--)
            {
                ...
                HierarchyDesignerFolder childFolder =
                    childTransform.GetComponent<HierarchyDesignerFolder>();
